...
FlowLines::addLines(cairo_t *cr, bool boundry, bool hatched, bool dashed,
  ...
  for(int i = 0; i < num_lines; i++) { //move loop outside for randomness of params per run???? ================
	...
	translation_y = -height/2.0 + (rand() % (int)ceil(height/2.0)) - (rand() % (int)ceil(height/2.0));
	...
	points = TextTransformations::make_points_wave(height, height, num_points, rand());
      ...
      TextTransformations::create_curved_path(cr,path,line,layout,width,height,0,0,points,true); // ==========================call boundry here???
