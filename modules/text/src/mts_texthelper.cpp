    ...
    namespace text{
        ...
        void 
            MTS_TextHelper::generateTxtPatch(cairo_surface_t *&textSurface, 
                ...
                const double *stretchProb=stretchProbability_[bgIndex];
                ...
                int x_pad = rng() % maxpad - maxpad/2;
                int y_pad = rng() % maxpad - maxpad/2;
                cout << "pad " << x_pad << " " << y_pad << endl;

                double scale = (rng()%5)/20.0+0.9;
                ...
                //Weight
                const double *weightProb=weightProbability_[bgIndex];
                int weight_prob = rng()%10000;
                ...
                if (distract) {
                    ...
                    for (int i=0;i<dis_num;i++) {
                        ...
                        distractText(cr_n, patchWidth, height, font2);
                    }
                }
        ...
        void 
            MTS_TextHelper::generateTxtSample (string &caption, cairo_surface_t *&textSurface, int height, int &width, int text_color, bool distract){
                ...
                if(sampleCaptions_->size()!=0){
                    caption = sampleCaptions_->at(rng()%sampleCaptions_->size());
        ...
        char
            MTS_TextHelper::randomChar() {
                ...
                char ch;
                int number = rng() % 5; // 1/5 chance number==0
